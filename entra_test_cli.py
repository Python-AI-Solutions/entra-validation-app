...
def handle_guide(args: argparse.Namespace) -> None:
    ...
    template = textwrap.dedent(
        ...
        tenants or app registrations without needing any backend.
        """
    ).strip()
    print(
        template.format(
...
def handle_report(args: argparse.Namespace) -> None:
    ...
    if failures:
        if any("AADSTS9002327" in entry.detail for entry in failures):
            print(
                "Hint: Microsoft Entra treated this app as a SPA. Serve the static site using `pixi run serve` "
...
def build_parser(defaults: EntraEnvDefaults, env_file: str) -> argparse.ArgumentParser:
    description = textwrap.dedent(
        f"""
        Validate Microsoft Entra OIDC credentials before integration.
